...
import { EraBadge, ProgressRing, RewardPill, StatPlaque } from '../../components/GameUI';
...
export function Dashboard({ games, missions, xp }: DashboardProps) {
  ...
  const activeMission = missions.find((mission) => !mission.completed)?.title ?? 'Choose a harder ladder mission';
    ...
    <>
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
        <Card className="min-h-[340px]">
          ...
          {games.length ? (
            <div className="h-64 rounded border border-gold/20 bg-[#0f0b08]/45 p-3">
            ...
            <div className="grid h-64 place-items-center rounded border border-dashed border-gold/30 bg-[#0f0b08]/45 px-6 text-center text-vellum">
        ...
        <div className="space-y-6">
          ...
          <Card>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-bronze">Current mission</p>
            ...
            <div className="mt-4">
              ...
              <div className="mt-3 h-3 overflow-hidden rounded border border-gold/20 bg-black/45">
        ...
        <Card className="aoe-plaque">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 font-display text-3xl text-parchment">Next ranked set</h2>
            </div>
            <div className="rounded border border-gold/30 bg-[#0f0b08]/55 px-3 py-2 text-right">
          ...
          <div className="mt-4 space-y-3">
            {recommendations.map((item) => (
              <div key={item} className="rounded border border-gold/25 bg-[#0f0b08]/45 p-3 text-sm text-parchment">{item}</div>
            ))}
          </div>
        </Card>
        <Card>
          ...
          <div className="mt-4 space-y-3">
            {games.slice(0, 4).map((game) => (
              <div key={game.id} className="flex items-center justify-between rounded border border-gold/20 bg-[#0f0b08]/45 p-3">
                <div>
